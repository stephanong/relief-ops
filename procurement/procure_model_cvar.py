""" Stochastic programming model for rice procurement with CVaR on all objectives
    This can suppersede stoch/stochcvar/stochcvarw by setting alpha-vector:
        alpha=[0.0,  0.0,  0.0] is the standard stochastic programming
        alpha=[0.95, 0.0,  0.0] is the CVaR-0.95 model on the shortage
        alpha=[0.0,  0.95, 0.0] is the CVaR-0.95 model on the waste    
...
def initiate_model(n, tau, b, r, t, 
                   scen, prob=[], alpha=[0.95, 0.0, 0.0], l=[], 
                   write=""): 
    """ build the stochastic model for procurement
        ...
        :param list alpha: confidence level of CVaR for each objective (length=3)
    ...
    model.varetas = etas = model.continuous_var(name="eta_s", lb=-model.infinity)
    model.varetaw = etaw = model.continuous_var(name="eta_w", lb=-model.infinity)
    model.varetab = etab = model.continuous_var(name="eta_b", lb=-model.infinity)
    
    model.varus = us = model.continuous_var_dict([k+1 for k in range(N)], name="u_s")
    model.varuw = uw = model.continuous_var_dict([k+1 for k in range(N)], name="u_w")
    model.varub = ub = model.continuous_var_dict([k+1 for k in range(N)], name="u_b")
                                                       
    # set objectives
    model.obj_shortage = cvar_shortage = etas + (1/(1-alpha[0]))*model.sum( (prob[k] if prob else 1/N)*us[k+1] for k in range(N) )
    model.obj_waste    = cvar_waste    = etaw + (1/(1-alpha[1]))*model.sum( (prob[k] if prob else 1/N)*uw[k+1] for k in range(N) )
    model.obj_budget   = cvar_budget   = etab + (1/(1-alpha[2]))*model.sum( (prob[k] if prob else 1/N)*ub[k+1] for k in range(N) )
    
    model.lstobj = [(model.minimize, cvar_shortage),
                    (model.minimize, cvar_waste),
                    (model.minimize, cvar_budget)]

    # model parts that exceed VaR(alpha)
    model.add_constraints(
        us[k+1] >= model.sum(s[i+1,k+1] for i in range(n+1)) - etas 
        for k in range(N) )
    model.add_constraints(
        uw[k+1] >= model.sum(x[i+1,tau,k+1] for i in range(n)) - etaw 
        for k in range(N) )
    model.add_constraints(
        ub[k+1] >= model.sum(scen[k][i][1]*y[i+1] for i in range(n)) - etab
        for k in range(N) )
...
#            scen.append(list(zip(d, p, a)))
        ...
        print("solution = {}, cvar-{} shortage = {}, cvar-{} waste = {}, cvar-{} budget = {}".format(
               solution, alpha[0], objective[0], alpha[1], objective[1], alpha[2], objective[2]))
