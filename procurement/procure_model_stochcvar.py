...
def initiate_model(n, tau, b, r, t, 
    ...
    """ build the stochastic model for procurement
        ...
        :param float alpha: confidence level of CVaR for the shortage objective
    ...
    # set objectives
    model.obj_shortage = cvar_shortage = eta + (1/(1-alpha))*model.sum( (prob[k] if prob else 1/N)*u[k+1] for k in range(N) )
    ...
    model.add_constraints(
        u[k+1] >= model.sum(s[i+1,k+1] for i in range(n+1)) - eta 
        for k in range(N) )
...
#            scen.append(list(zip(d, p, a)))
        ...
        print("solution = {}, cvar-{} shortage = {}, expected (waste = {}, used budget = {})".format(
               solution, alpha, objective[0], objective[1], objective[2]))
